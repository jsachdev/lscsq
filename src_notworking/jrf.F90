...
!     ------------------------------------------------------------------
!
subroutine lscsq_VmaxCalc(arrys)
   use pl_types 
  use iso_c_binding, only : fp => c_double
  use lscsq_mod, only : Vparmaxn,Vparmaxp, vpar, npsi
  ...
  do ir=1,nrays
     if (ok_ray(ir).eq.1) then
        do iz=1,nzones
           iv = arrys(iz,ir)%ivind
           ip = arrys(iz,ir)%izind
...
!
      do i=1,NumJs
         xwk(i) = (REAL(i,kind=fp)+0.5_fp)/ Jp1
      enddo
      xwk0 = 0.5_fp/Jp1
      ...
      Bwk(1)     = Bwk(1) + xwk0*(10_fp-BC1) + xwk(1)
