subroutine lscsq_PdepCalc(arrys)
 ...
 SUBROUTINE lscsq_RfDamp(arrys)
   ...
   type(storeAry), dimension(nzones, nrays), intent(in) :: arrys
 ...
 !     velocity zone arrys%izind(izone, iray), arrys%ivind(izone, iray) and use
   ...
   do iry = 1, nrays
     if (ok_ray(iry).eq.1) then
        do izn = 1, nzones-1
           ...
           ! If  arrys%ivind=0, the ray was stopped before reaching this izone (izn).
           ! No calculation is appropriate.
           if (iv.EQ.0) cycle    
           ips = arrys(izn,iry)%izind
 ...
 !     -----------------------------------------------------------------
 SUBROUTINE lscsq_RayDamp(arrys)
   use pl_types 
   use iso_c_binding, only: fp => c_double, c_int
   use lscsq_mod, only :  nrays, nzones, ok_ray
   use lscsq_mod, only : rFudgDmp, power, dfdv, ivzero
  ...
  do iry = 1, nrays
    if (ok_ray(iry).eq.1) then
     ...
     do i=1,nzones-1
        ...
        dum = 1.0_fp + arrys(i, iry)%dlnPdsX
        ...
        if (dum .LT. MxdPdz) then
           write(ErrMsg,'( '' Mxw damp! iry,izn,iv,dlnPdsX :'',i4,i4,i4,1pe11.3)')      &
                           iry, i, arrys(i, iry)%ivind,arrys(i, iry)%dlnPdsX
           dum = MxdPdZ
        endif
     ...
     do i=1,nzones-1
        ...
        dum = arrys(i, iry)%dlnPdsK*dfdv(arrys(i, iry)%ivind,arrys(i, iry)%izind,2)
